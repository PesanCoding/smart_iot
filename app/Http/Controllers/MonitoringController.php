...
class MonitoringController extends Controller
{
        ...
        $role = Auth::user()->role_id;
            if ($role == 1) {
                ...
                $Device = Devices::pluck('nama_devices','id_user');
                 ...
                 $user = Auth::user()->id;
                    if ($request->ajax()) {
                        $data = Monitoring::select('id','id_user','suhu_udara','kel_udara','kel_tanah','waktu')
                        ->orderBy('id','DESC')
                        ->where('id_user','=',$user)
                        ->get();
                        return Datatables::of($data)->addIndexColumn()
                            ...
                            ->addColumn('suhu_udara', function($data){
                                return $data->suhu_udara .' C';
                            })
                            ->addColumn('kel_udara', function($data){
                                return $data->kel_udara .' %';
                            })
                            ->addColumn('kel_tanah', function($data){
                                return $data->kel_tanah .' %';
                            })
            ...
            if ($request->id_device == $data) {
                $insert = DB::table('sensors');
                    $insert->insert([
                        ...
                        'kel_udara'     =>$request->kel_udara,
                        'kel_tanah'     =>$request->kel_tanah,
                    ]);
    ...
    {
        $user = Auth::user()->id;
        $data = Monitoring::where('id_user','=',$user)->count();
            $batas = 20;
                if ($data > $batas) {
                    $dataHapus = $data - $batas;
                    Monitoring::orderBy('id','asc')->limit($dataHapus)->delete();
                    return response()->json(['message' => "data telah dihapus"]);
                }else{
                    return response()->json(['message' => "Tidak Perlu Menghapus Data"]);
                }
    }
}
