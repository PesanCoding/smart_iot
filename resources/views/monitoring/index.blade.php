                ...
                <div class="card-body">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>Kel. Udara</th>
                            <th>Kel. Tanah</th>
                            <th>Aksi <button class="btn btn-sm btn-danger d-none" id="deleteAllBtn">Delete All</button></th>
                        </tr>
                    </thead>
                    <tbody></tbody>
                </table>
...
$.ajaxSetup({
   ...
   var table = $('.mont_datatable').DataTable({
        ...
        columns: [
            ...
            {data: 'suhu_udara', name: 'suhu_udara'},
            {data: 'kel_udara', name: 'kel_udara'},
            {data: 'kel_tanah', name: 'kel_tanah'},
        ...
        $('input[name="country_checkbox"]:checked').each(function(){
            ...
            if(checked.length > 0){
                swal.fire({
                    ...
                    if(result.value){
                        $.post(url,{countries_ids:checked},function(data){
                            if(data.code == 1){
    ...
    $(document).on('click','#deleteSensorBtn',function(){
        ...
        var url = '<?= route("delete.sensor") ?>';
         swal.fire({
                ...
                if(result.value){
                    $.post(url,{sensor_id:sensor_id}, function(data){
                        if(data.code == 1){
